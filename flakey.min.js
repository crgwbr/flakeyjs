...
Model.get=function(id){var m,obj;obj=Flakey.models.backend_controller.get(this.model_name,id);if(!obj)return;m=new this;m["import"](obj);return m};Model.prototype["delete"]=function(){var event_key;Flakey.models.backend_controller["delete"](this.constructor.model_name,this.id);event_key="model_"+this.constructor.model_name.toLowerCase()+"_updated";return Flakey.events.trigger(event_key,void 0)};Model.prototype.rollback=function(version_id){var exists,i,latest,version,_i,_j,_len,_len2,_ref,_ref2;if(parseInt(version_id)<
this.versions.length){_ref=Array(parseInt(version_id));for(_i=0,_len=_ref.length;_i<_len;_i++){i=_ref[_i];this.pop_version()}}else{exists=false;_ref2=this.versions;for(_j=0,_len2=_ref2.length;_j<_len2;_j++){version=_ref2[_j];if(version.version_id===version_id)exists=true}if(!exists)throw new ReferenceError("Version "+version_id+" does not exist.");latest=this.versions[this.versions.length-1];while(latest.version_id!==version_id&&this.versions.length>1)this.pop_version()}this["import"]({id:this.id,
