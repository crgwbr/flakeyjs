...
this.read=Flakey.settings.read_backend||"memory";this.load_logs()}BackendController.prototype.all=function(name){return this.backends[this.read].interface.all(name)};BackendController.prototype.get=function(name,id){return this.backends[this.read].interface.get(name,id)};BackendController.prototype.find=function(name,query){return this.backends[this.read].interface.find(name,query)};BackendController.prototype.save=function(name,id,versions,backends){var backend,bname,log_msg;if(backends==null)backends=
this.backends;for(bname in backends){if(!__hasProp.call(backends,bname))continue;backend=backends[bname];log_msg="save"+this.delim+JSON.stringify([name,id,versions]);if(backend.pending_log.length){backend.pending_log.push(log_msg);this.commit_logs();this.exec_log();return false}if(!backend.interface.save(name,id,versions)){backend.pending_log.push(log_msg);this.commit_logs();return false}}return true};BackendController.prototype["delete"]=function(name,id,backends){var backend,bname,log_msg;if(backends==
null)backends=this.backends;for(bname in backends){if(!__hasProp.call(backends,bname))continue;backend=backends[bname];log_msg="delete"+this.delim+JSON.stringify([name,id]);if(backend.pending_log.length){backend.pending_log.push(log_msg);this.commit_logs();this.exec_log();return false}if(!backend.interface["delete"](name,id)){backend.pending_log.push(log_msg);this.commit_logs();return false}}return true};BackendController.prototype.exec_log=function(){var action,backend,bends,fn,log,msg,name,params,
_i,_len,_ref;_ref=this.backends;for(name in _ref){if(!__hasProp.call(_ref,name))continue;backend=_ref[name];log=backend.pending_log;for(_i=0,_len=log.length;_i<_len;_i++){msg=log[_i];if(msg!=null){action=msg.split(this.delim);fn=Flakey.models.backend_controller.backends[name].interface[action[0]];params=JSON.parse(action[1]);bends={};bends[name]=backend;params.push(bends);if(fn.apply(Flakey.models.backend_controller.backends[name].interface,params))backend.pending_log.shift();else break}}}return this.commit_logs()};
BackendController.prototype.commit_logs=function(backends){var backend,name;if(backends==null)backends=this.backends;for(name in backends){if(!__hasProp.call(backends,name))continue;backend=backends[name];localStorage[backend.log_key]=JSON.stringify(backend.pending_log)}return true};BackendController.prototype.load_logs=function(backends){var backend,name;if(backends==null)backends=this.backends;for(name in backends){if(!__hasProp.call(backends,name))continue;backend=backends[name];if(!(localStorage[backend.log_key]!=
null))break;backend.pending_log=JSON.parse(localStorage[backend.log_key])}return true};BackendController.prototype.sync=function(name,backends){var backend,bname,event_key,item,key,output,store,_i,_len,_ref,_ref2;if(backends==null)backends=this.backends;store={};for(bname in backends){if(!__hasProp.call(backends,bname))continue;backend=backends[bname];_ref=backend.interface.all(name);for(_i=0,_len=_ref.length;_i<_len;_i++){item=_ref[_i];if(_ref2=item.id,__indexOf.call(Object.keys(store),_ref2)>=0)store[item.id].versions=
this.merge_version_lists(item.versions,store[item.id].versions);else store[item.id]=item}}output=[];for(key in store){if(!__hasProp.call(store,key))continue;item=store[key];output.push(item)}for(bname in backends){if(!__hasProp.call(backends,bname))continue;backend=backends[bname];backend.interface._write(name,output)}event_key="model_"+name.toLowerCase()+"_updated";return Flakey.events.trigger(event_key,void 0)};BackendController.prototype.merge_version_lists=function(a,b){var key,keys,output,rev,
temp,value,_i,_j,_len,_len2,_ref,_ref2,_ref3;temp={};_ref=a.concat(b);for(_i=0,_len=_ref.length;_i<_len;_i++){rev=_ref[_i];if(_ref2=rev.time,__indexOf.call(Object.keys(temp),_ref2)>=0)if(rev.id!==temp[rev.time].id){_ref3=rev.fields;for(key in _ref3){if(!__hasProp.call(_ref3,key))continue;value=_ref3[key];temp[rev.time].fields[key]=value}}else temp[rev.time]=rev;else temp[rev.time]=rev}keys=Object.keys(temp);keys.sort(function(a,b){return a-b});output=[];for(_j=0,_len2=keys.length;_j<_len2;_j++){key=
keys[_j];output.push(temp[key])}return output};return BackendController}();Backend=function(){function Backend(){}Backend.prototype.all=function(name){var store;store=this._read(name);if(!store)return[];return store};Backend.prototype.get=function(name,id){var index,store;store=this._read(name);index=this._query_by_id(name,id);if(index===-1)return;return store[index]};Backend.prototype.find=function(name,query){var i,iset,set,store,_i,_len;store=this._read(name);iset=this._query(name,query);set=[];
for(_i=0,_len=iset.length;_i<_len;_i++){i=iset[_i];set.push(store[i])}return set};Backend.prototype.save=function(name,id,versions){var index,obj,store;store=this._read(name);if(!store)store=[];index=this._query_by_id(name,id);obj={id:id,versions:versions};if(index===-1)store.push(obj);else store[index]=obj;return this._write(name,store)};Backend.prototype["delete"]=function(name,id){var index,store;store=this._read(name);index=this._query_by_id(name,id);if(index===-1)return true;store.splice(index,
