...
a.charCodeAt(0).toString(16)).slice(-4)});if(/^[\],:{}\s]*$/.test(text.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,"@").replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,"]").replace(/(?:^|:|,)(?:\s*\[)+/g,""))){j=eval("("+text+")");return typeof reviver==="function"?walk({"":j},""):j}throw new SyntaxError("JSON.parse");}})();var $,Backend,BackendController,Controller,Events,Flakey,JSON,LocalBackend,MemoryBackend,Model,ServerBackend,Stack,Template,get_template;var __hasProp=
...
Flakey={diff_patch:new diff_match_patch,settings:{diff_text:true,container:void 0,read_backend:"memory",base_model_endpoint:null},status:{server_online:void 0}};jQuery.noConflict();$=Flakey.$=jQuery;JSON=Flakey.JSON=JSON;Flakey.init=function(config){var key,value;for(key in config){if(!__hasProp.call(config,key))continue;value=config[key];Flakey.settings[key]=value}return Flakey.models.backend_controller=new Flakey.models.BackendController};if(window)window.Flakey=Flakey;Flakey.util={async:function(fn){return setTimeout(fn,
...
value}}Model.all=function(){var m,obj,set,_i,_len,_ref;set=[];_ref=Flakey.models.backend_controller.all(this.model_name);for(_i=0,_len=_ref.length;_i<_len;_i++){obj=_ref[_i];m=new this;m["import"](obj);set.push(m)}return set};Model.prototype["delete"]=function(){return Flakey.models.backend_controller["delete"](this.constructor.model_name,this.id)};Model.prototype.diff=function(new_obj,old_obj){var key,patches,save,_i,_len,_ref;save={};_ref=this.constructor.fields;for(_i=0,_len=_ref.length;_i<_len;_i++){key=
_ref[_i];if(!Flakey.util.deep_compare(new_obj[key],old_obj[key]))switch(new_obj[key].constructor){case Object:save[key]=$.extend(true,{},new_obj[key]);break;case Array:save[key]=$.extend(true,[],new_obj[key]);break;case String:old_obj[key]=old_obj[key]!=null?old_obj[key].toString():"";if(Flakey.settings.diff_text){patches=Flakey.diff_patch.patch_make(old_obj[key],new_obj[key]);save[key]={constructor:"Patch",patch_text:Flakey.diff_patch.patch_toText(patches)}}else save[key]=new_obj[key];break;default:save[key]=
new_obj[key]}}return save};Model.prototype["export"]=function(){var field,obj,_i,_len,_ref;obj={};_ref=this.constructor.fields;for(_i=0,_len=_ref.length;_i<_len;_i++){field=_ref[_i];obj[field]=this[field]}return obj};Model.prototype.evolve=function(version_id){var key,obj,patches,rev,saved,value,versions,_i,_len,_ref;obj={};saved=Flakey.models.backend_controller.get(this.constructor.model_name,this.id);versions=saved!=null?saved.versions:{};for(_i=0,_len=versions.length;_i<_len;_i++){rev=versions[_i];
_ref=rev.fields;for(key in _ref){if(!__hasProp.call(_ref,key))continue;value=_ref[key];switch(value.constructor){case "Patch":patches=Flakey.diff_patch.patch_fromText(value.patch_text);obj[key]=Flakey.diff_patch.patch_apply(patches,obj[key]||"")[0];break;case Object:obj[key]=$.extend(true,{},value);break;case Array:obj[key]=$.extend(true,[],value);break;default:obj[key]=value}}if(version_id!==void 0&&version_id===rev.version_id)return obj}return obj};Model.find=function(query){var ar,item,m,set,_i,
_len;ar=Flakey.models.backend_controller.find(this.model_name,query);if(!ar.length)return[];set=[];for(_i=0,_len=ar.length;_i<_len;_i++){item=ar[_i];m=new this;m["import"](item);set.push(m)}return set};Model.get=function(id){var m,obj;obj=Flakey.models.backend_controller.get(this.model_name,id);if(!obj)return;m=new this;m["import"](obj);return m};Model.prototype["import"]=function(obj){var key,value,_ref,_results;this.versions=obj.versions;this.id=obj.id;_ref=this.evolve();_results=[];for(key in _ref){if(!__hasProp.call(_ref,
key))continue;value=_ref[key];_results.push(this[key]=value)}return _results};Model.prototype.pop_version=function(){return this.versions.pop()};Model.prototype.push_version=function(diff){var version,version_id;version_id=Flakey.util.guid();version={version_id:version_id,time:+new Date,fields:$.extend(true,{},diff)};Object.freeze(version);return this.versions.push(version)};Model.prototype.save=function(callback){var diff,event_key,new_obj,old_obj;var _this=this;new_obj=this["export"]();old_obj=
this.evolve();diff=this.diff(new_obj,old_obj);if(Object.keys(diff).length>0){this.push_version(diff);Flakey.util.async(function(){Flakey.models.backend_controller.save(_this.constructor.model_name,_this.id,_this.versions);if(callback!=null)return callback()})}else if(callback!=null)callback();event_key="model_"+this.constructor.model_name.toLowerCase()+"_updated";Flakey.events.trigger(event_key,void 0,{model:this});return true};return Model}();BackendController=function(){function BackendController(){this.delim=
":::";this.backends={memory:{log_key:"flakey-memory-log",pending_log:[],"interface":new MemoryBackend},local:{log_key:"flakey-local-log",pending_log:[],"interface":new LocalBackend}};if(Flakey.settings.base_model_endpoint)this.backends["server"]={log_key:"flakey-server-log",pending_log:[],"interface":new ServerBackend};this.read=Flakey.settings.read_backend||"memory";this.load_logs()}BackendController.prototype.all=function(name){return this.backends[this.read].interface.all(name)};BackendController.prototype.get=
function(name,id){return this.backends[this.read].interface.get(name,id)};BackendController.prototype.find=function(name,query){return this.backends[this.read].interface.find(name,query)};BackendController.prototype.save=function(name,id,versions,backends){var backend,bname,log_msg;if(backends==null)backends=this.backends;for(bname in backends){if(!__hasProp.call(backends,bname))continue;backend=backends[bname];log_msg="save"+this.delim+JSON.stringify([name,id,versions]);if(backend.pending_log.length){backend.pending_log.push(log_msg);
...
BackendController.prototype.load_logs=function(backends){var backend,name;if(backends==null)backends=this.backends;for(name in backends){if(!__hasProp.call(backends,name))continue;backend=backends[name];if(!(localStorage[backend.log_key]!=null))break;backend.pending_log=JSON.parse(localStorage[backend.log_key])}return true};BackendController.prototype.sync=function(name,backends){var backend,bname,item,key,output,store,_i,_len,_ref,_ref2,_results;if(backends==null)backends=this.backends;store={};
...
bname))continue;backend=backends[bname];_results.push(backend.interface._write(name,output))}return _results};BackendController.prototype.merge_version_lists=function(a,b){var key,keys,output,rev,temp,value,_i,_j,_len,_len2,_ref,_ref2,_ref3;temp={};_ref=a.concat(b);for(_i=0,_len=_ref.length;_i<_len;_i++){rev=_ref[_i];if(_ref2=rev.time,__indexOf.call(Object.keys(temp),_ref2)>=0)if(rev.id!==temp[rev.time].id){_ref3=rev.fields;for(key in _ref3){if(!__hasProp.call(_ref3,key))continue;value=_ref3[key];
temp[rev.time].fields[key]=value}}else temp[rev.time]=rev;else temp[rev.time]=rev}keys=Object.keys(temp);keys.sort(function(a,b){return a-b});output=[];for(_j=0,_len2=keys.length;_j<_len2;_j++){key=keys[_j];output.push(temp[key])}return output};return BackendController}();Backend=function(){function Backend(){}Backend.prototype.all=function(name){var store;store=this._read(name);if(!store)return[];return store};Backend.prototype.get=function(name,id){var index,store;store=this._read(name);index=this._query_by_id(name,
id);if(index===-1)return;return store[index]};Backend.prototype.find=function(name,query){var i,iset,set,store,_i,_len;store=this._read(name);iset=this._query(name,query);set=[];for(_i=0,_len=iset.length;_i<_len;_i++){i=iset[_i];set.push(store[i])}return set};Backend.prototype.save=function(name,id,versions){var index,obj,store;store=this._read(name);if(!store)store=[];index=this._query_by_id(name,id);obj={id:id,versions:versions};if(index===-1)store.push(obj);else store[index]=obj;return this._write(name,
store)};Backend.prototype["delete"]=function(name,id){var index,store;store=this._read(name);index=this._query_by_id(name,id);if(index===-1)return true;store.splice(index,1);return this._write(name,store)};Backend.prototype._query=function(name,query){var i,key,obj,rendered,set,store,value,_i,_len;store=this._read(name);if(!store)return[];set=[];i=0;for(_i=0,_len=store.length;_i<_len;_i++){obj=store[_i];rendered=this._render_obj(obj);for(key in query){if(!__hasProp.call(query,key))continue;value=
query[key];if(rendered[key]===value)set.push(i)}i++}return set};Backend.prototype._query_by_id=function(name,id){var i,obj,store,_i,_len;store=this._read(name);if(!store)return-1;i=0;for(_i=0,_len=store.length;_i<_len;_i++){obj=store[_i];if(obj.id===id)return i;i++}return-1};Backend.prototype._render_obj=function(obj){var key,output,patches,rev,value,_i,_len,_ref,_ref2;output={};_ref=obj.versions;for(_i=0,_len=_ref.length;_i<_len;_i++){rev=_ref[_i];_ref2=rev.fields;for(key in _ref2){if(!__hasProp.call(_ref2,
