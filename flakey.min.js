...
key))continue;value=_ref2[key];if(value.constructor==="Patch"){patches=Flakey.diff_patch.patch_fromText(value.patch_text);output[key]=Flakey.diff_patch.patch_apply(patches,obj[key]||"")[0]}else output[key]=value}}return output};return Backend}();MemoryBackend=function(_super){__extends(MemoryBackend,_super);function MemoryBackend(){if(!window.memcache)window.memcache={}}MemoryBackend.prototype._read=function(name){return window.memcache[name]};MemoryBackend.prototype._write=function(name,store){window.memcache[name]=
store;return true};return MemoryBackend}(Backend);LocalBackend=function(_super){__extends(LocalBackend,_super);function LocalBackend(){this.prefix="flakey-"}LocalBackend.prototype._read=function(name){var store;if(!localStorage[this.prefix+name])localStorage[this.prefix+name]=JSON.stringify([]);store=JSON.parse(localStorage[this.prefix+name]);return store};LocalBackend.prototype._write=function(name,store){localStorage[this.prefix+name]=JSON.stringify(store);return true};return LocalBackend}(Backend);
ServerBackend=function(_super){__extends(ServerBackend,_super);function ServerBackend(){this.server_cache={}}ServerBackend.prototype.build_endpoint_url=function(name,id,params){var querystring,url;url=""+Flakey.settings.base_model_endpoint+"/"+name;if(id!=null)url+="/"+id;if(params!=null&&params.constructor===Object){querystring=Flakey.util.querystring.build(params);url+="?"+querystring}return url};ServerBackend.prototype.all=function(name){var obj,store,_i,_len;store=false;$.ajax({async:false,url:this.build_endpoint_url(name),
