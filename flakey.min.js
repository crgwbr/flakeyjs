...
Model.prototype.save=function(callback){var diff,new_obj,old_obj,_this=this;new_obj=this["export"]();old_obj=this.evolve();diff=this.diff(new_obj,old_obj);if(Object.keys(diff).length>0){this.push_version(diff);Flakey.util.async(function(){var event_key;Flakey.models.backend_controller.save(_this.constructor.model_name,_this.id,_this.versions);if(callback!=null)callback();event_key="model_"+_this.constructor.model_name.toLowerCase()+"_updated";return Flakey.events.trigger(event_key,void 0)})}else if(callback!=
