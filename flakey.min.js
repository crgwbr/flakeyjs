...
version={version_id:version_id,time:+new Date,fields:$.extend(true,{},diff)};Object.freeze(version);return this.versions.push(version)};Model.prototype.save=function(){var diff,new_obj,old_obj,_this=this;new_obj=this["export"]();old_obj=this.evolve();diff=this.diff(new_obj,old_obj);if(Object.keys(diff).length>0){this.push_version(diff);return Flakey.util.async(function(){return Flakey.models.backend_controller.save(_this.constructor.model_name,_this.id,_this.versions)})}};return Model}();BackendController=
function(){function BackendController(){this.delim=":::";this.backends={memory:{log_key:"flakey-memory-log",pending_log:[],"interface":new MemoryBackend},local:{log_key:"flakey-local-log",pending_log:[],"interface":new LocalBackend}};if(Flakey.settings.base_model_endpoint)this.backends["server"]={log_key:"flakey-server-log",pending_log:[],"interface":new ServerBackend};this.read=Flakey.settings.read_backend||"memory";this.load_logs()}BackendController.prototype.all=function(name){return this.backends[this.read].interface.all(name)};
BackendController.prototype.get=function(name,id){return this.backends[this.read].interface.get(name,id)};BackendController.prototype.find=function(name,query){return this.backends[this.read].interface.find(name,query)};BackendController.prototype.save=function(name,id,versions,backends){var backend,bname,log_msg;if(backends==null)backends=this.backends;for(bname in backends){if(!__hasProp.call(backends,bname))continue;backend=backends[bname];log_msg="save"+this.delim+JSON.stringify([name,id,versions]);
if(backend.pending_log.length){backend.pending_log.push(log_msg);this.commit_logs();this.exec_log();return false}if(!backend.interface.save(name,id,versions)){backend.pending_log.push(log_msg);this.commit_logs();return false}}return true};BackendController.prototype["delete"]=function(name,id,backends){var backend,bname,log_msg;if(backends==null)backends=this.backends;for(bname in backends){if(!__hasProp.call(backends,bname))continue;backend=backends[bname];log_msg="delete"+this.delim+JSON.stringify([name,
id]);if(backend.pending_log.length){backend.pending_log.push(log_msg);this.commit_logs();this.exec_log();return false}if(!backend.interface["delete"](name,id)){backend.pending_log.push(log_msg);this.commit_logs();return false}}return true};BackendController.prototype.exec_log=function(){var action,backend,bends,fn,log,msg,name,params,_i,_len,_ref;_ref=this.backends;for(name in _ref){if(!__hasProp.call(_ref,name))continue;backend=_ref[name];log=backend.pending_log;for(_i=0,_len=log.length;_i<_len;_i++){msg=
log[_i];action=msg.split(this.delim);fn=Flakey.models.backend_controller.backends[name].interface[action[0]];params=JSON.parse(action[1]);bends={};bends[name]=backend;params.push(bends);if(fn.apply(Flakey.models.backend_controller.backends[name].interface,params))backend.pending_log.shift();else break}}return this.commit_logs()};BackendController.prototype.commit_logs=function(backends){var backend,name;if(backends==null)backends=this.backends;for(name in backends){if(!__hasProp.call(backends,name))continue;
backend=backends[name];localStorage[backend.log_key]=JSON.stringify(backend.pending_log)}return true};BackendController.prototype.load_logs=function(backends){var backend,name;if(backends==null)backends=this.backends;for(name in backends){if(!__hasProp.call(backends,name))continue;backend=backends[name];if(!(localStorage[backend.log_key]!=null))break;backend.pending_log=JSON.parse(localStorage[backend.log_key])}return true};BackendController.prototype.sync=function(name,backends){var backend,bname,
item,key,output,store,_i,_len,_ref,_ref2,_results;if(backends==null)backends=this.backends;store={};for(bname in backends){if(!__hasProp.call(backends,bname))continue;backend=backends[bname];_ref=backend.interface.all(name);for(_i=0,_len=_ref.length;_i<_len;_i++){item=_ref[_i];if(_ref2=item.id,__indexOf.call(Object.keys(store),_ref2)>=0)store[item.id].versions=this.merge_version_lists(item.versions,store[item.id].versions);else store[item.id]=item}}output=[];for(key in store){if(!__hasProp.call(store,
key))continue;item=store[key];output.push(item)}_results=[];for(bname in backends){if(!__hasProp.call(backends,bname))continue;backend=backends[bname];_results.push(backend.interface._write(name,output))}return _results};BackendController.prototype.merge_version_lists=function(a,b){var key,keys,output,rev,temp,value,_i,_j,_len,_len2,_ref,_ref2,_ref3;temp={};_ref=a.concat(b);for(_i=0,_len=_ref.length;_i<_len;_i++){rev=_ref[_i];if(_ref2=rev.time,__indexOf.call(Object.keys(temp),_ref2)>=0)if(rev.id!==
temp[rev.time].id){_ref3=rev.fields;for(key in _ref3){if(!__hasProp.call(_ref3,key))continue;value=_ref3[key];temp[rev.time].fields[key]=value}}else temp[rev.time]=rev;else temp[rev.time]=rev}keys=Object.keys(temp);keys.sort(function(a,b){return a-b});output=[];for(_j=0,_len2=keys.length;_j<_len2;_j++){key=keys[_j];output.push(temp[key])}return output};return BackendController}();Backend=function(){function Backend(){}Backend.prototype.all=function(name){var store;store=this._read(name);if(!store)return[];
return store};Backend.prototype.get=function(name,id){var index,store;store=this._read(name);index=this._query_by_id(name,id);if(index===-1)return;return store[index]};Backend.prototype.find=function(name,query){var i,iset,set,store,_i,_len;store=this._read(name);iset=this._query(name,query);set=[];for(_i=0,_len=iset.length;_i<_len;_i++){i=iset[_i];set.push(store[i])}return set};Backend.prototype.save=function(name,id,versions){var index,obj,store;store=this._read(name);if(!store)store=[];index=this._query_by_id(name,
id);obj={id:id,versions:versions};if(index===-1)store.push(obj);else store[index]=obj;return this._write(name,store)};Backend.prototype["delete"]=function(name,id){var index,store;store=this._read(name);index=this._query_by_id(name,id);if(index===-1)return true;store.splice(index,1);return this._write(name,store)};Backend.prototype._query=function(name,query){var i,key,obj,rendered,set,store,value,_i,_len;store=this._read(name);if(!store)return[];set=[];i=0;for(_i=0,_len=store.length;_i<_len;_i++){obj=
store[_i];rendered=this._render_obj(obj);for(key in query){if(!__hasProp.call(query,key))continue;value=query[key];if(rendered[key]===value)set.push(i)}i++}return set};Backend.prototype._query_by_id=function(name,id){var i,obj,store,_i,_len;store=this._read(name);if(!store)return-1;i=0;for(_i=0,_len=store.length;_i<_len;_i++){obj=store[_i];if(obj.id===id)return i;i++}return-1};Backend.prototype._render_obj=function(obj){var key,output,patches,rev,value,_i,_len,_ref,_ref2;output={};_ref=obj.versions;
for(_i=0,_len=_ref.length;_i<_len;_i++){rev=_ref[_i];_ref2=rev.fields;for(key in _ref2){if(!__hasProp.call(_ref2,key))continue;value=_ref2[key];if(value.constructor==="Patch"){patches=Flakey.diff_patch.patch_fromText(value.patch_text);output[key]=Flakey.diff_patch.patch_apply(patches,obj[key]||"")[0]}else output[key]=value}}return output};return Backend}();MemoryBackend=function(_super){__extends(MemoryBackend,_super);function MemoryBackend(){if(!window.memcache)window.memcache={}}MemoryBackend.prototype._read=
function(name){return window.memcache[name]};MemoryBackend.prototype._write=function(name,store){window.memcache[name]=store;return true};return MemoryBackend}(Backend);LocalBackend=function(_super){__extends(LocalBackend,_super);function LocalBackend(){this.prefix="flakey-"}LocalBackend.prototype._read=function(name){var store;if(!localStorage[this.prefix+name])localStorage[this.prefix+name]=JSON.stringify([]);store=JSON.parse(localStorage[this.prefix+name]);return store};LocalBackend.prototype._write=
...
ServerBackend.prototype.all=function(name){var obj,store,_i,_len;store=false;$.ajax({async:false,url:this.build_endpoint_url(name),dataType:"json",error:function(){return Flakey.status.server_online=false},success:function(data){Flakey.status.server_online=true;return store=data},type:"GET"});for(_i=0,_len=store.length;_i<_len;_i++){obj=store[_i];this.save_to_cache(obj)}return store};ServerBackend.prototype.get=function(name,id){var obj;obj=false;$.ajax({async:false,url:this.build_endpoint_url(name,
id),dataType:"json",error:function(){return Flakey.status.server_online=false},success:function(data){Flakey.status.server_online=true;return obj=data},type:"GET"});this.save_to_cache(obj);return obj};ServerBackend.prototype.get_from_cache=function(id){return this.server_cache[id]};ServerBackend.prototype.save=function(name,id,versions,force_write){var cached_obj,proposed_obj,status;proposed_obj={id:id,versions:versions};cached_obj=this.get_from_cache(id);if(Flakey.util.deep_compare(proposed_obj,
