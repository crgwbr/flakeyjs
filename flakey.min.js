...
j||i.body&&i.body[d]||j}if(a.nodeType===9){i=a.documentElement;if(i[d]>=i[e])return i[d];return Math.max(a.body[e],i[e],a.body[g],i[g])}if(h===b){k=f.css(a,c),l=parseFloat(k);return f.isNumeric(l)?l:k}f(a).css(c,h)},c,a,arguments.length,null)}}),a.jQuery=a.$=f,typeof define=="function"&&define.amd&&define.amd.jQuery&&define("jquery",[],function(){return f})})(window);(function(jQuery){jQuery.hotkeys={version:"0.8",specialKeys:{8:"backspace",9:"tab",13:"return",16:"shift",17:"ctrl",18:"alt",19:"pause",
...
"=":"+",";":": ","'":'"',",":"<",".":">","/":"?","\\":"|"}};function keyHandler(handleObj){if(typeof handleObj.data!=="string")return;var origHandler=handleObj.handler,keys=handleObj.data.toLowerCase().split(" ");handleObj.handler=function(event){if(this!==event.target&&(/textarea|select/i.test(event.target.nodeName)||event.target.type==="text"))return;var special=event.type!=="keypress"&&jQuery.hotkeys.specialKeys[event.which],character=String.fromCharCode(event.which).toLowerCase(),key,modif="",
possible={};if(event.altKey&&special!=="alt")modif+="alt+";if(event.ctrlKey&&special!=="ctrl")modif+="ctrl+";if(event.metaKey&&!event.ctrlKey&&special!=="meta")modif+="meta+";if(event.shiftKey&&special!=="shift")modif+="shift+";if(special)possible[modif+special]=true;else{possible[modif+character]=true;possible[modif+jQuery.hotkeys.shiftNums[character]]=true;if(modif==="shift+")possible[jQuery.hotkeys.shiftNums[character]]=true}for(var i=0,l=keys.length;i<l;i++)if(possible[keys[i]])return origHandler.apply(this,
arguments)}}jQuery.each(["keydown","keyup","keypress"],function(){jQuery.event.special[this]={add:keyHandler}})})(jQuery);var JSON;if(!JSON)JSON={};(function(){function f(n){return n<10?"0"+n:n}if(typeof Date.prototype.toJSON!=="function"){Date.prototype.toJSON=function(key){return isFinite(this.valueOf())?this.getUTCFullYear()+"-"+f(this.getUTCMonth()+1)+"-"+f(this.getUTCDate())+"T"+f(this.getUTCHours())+":"+f(this.getUTCMinutes())+":"+f(this.getUTCSeconds())+"Z":null};String.prototype.toJSON=Number.prototype.toJSON=
...
key))continue;value=query[key];query[key]=new RegExp(value,"g")}set=[];i=0;for(_i=0,_len=store.length;_i<_len;_i++){obj=store[_i];rendered=this._render_obj(obj);match=false;for(key in query){if(!__hasProp.call(query,key))continue;value=query[key];if(value.exec(rendered[key]))match=true}if(match)set.push(i);i++}return set};return Backend}();MemoryBackend=function(){__extends(MemoryBackend,Backend);function MemoryBackend(){if(!window.memcache)window.memcache={}}MemoryBackend.prototype._read=function(name){return $.extend(true,
[],window.memcache[name])};MemoryBackend.prototype._write=function(name,store){window.memcache[name]=$.extend(true,[],store);return true};return MemoryBackend}();LocalBackend=function(){__extends(LocalBackend,Backend);function LocalBackend(){this.prefix="flakey-"}LocalBackend.prototype._read=function(name){var store;if(!localStorage[this.prefix+name])localStorage[this.prefix+name]=JSON.stringify([]);store=JSON.parse(localStorage[this.prefix+name]);return store};LocalBackend.prototype._write=function(name,
store){localStorage[this.prefix+name]=JSON.stringify(store);return true};return LocalBackend}();ServerBackend=function(){__extends(ServerBackend,Backend);function ServerBackend(){this.server_cache={}}ServerBackend.prototype.build_endpoint_url=function(name,id,params){var querystring,url;url=""+Flakey.settings.base_model_endpoint+"/"+name;if(id!=null)url+="/"+id;if(params!=null&&params.constructor===Object){querystring=Flakey.util.querystring.build(params);url+="?"+querystring}return url};ServerBackend.prototype.all=
function(name){var obj,store,_i,_len;store=false;$.ajax({async:false,url:this.build_endpoint_url(name),dataType:"json",error:function(){return Flakey.status.server_online=false},success:function(data){Flakey.status.server_online=true;return store=data},type:"GET"});for(_i=0,_len=store.length;_i<_len;_i++){obj=store[_i];this.save_to_cache(name,obj.id,obj.versions)}return store};ServerBackend.prototype.get=function(name,id){var obj;obj=false;$.ajax({async:false,url:this.build_endpoint_url(name,id),
dataType:"json",error:function(){return Flakey.status.server_online=false},success:function(data){Flakey.status.server_online=true;return obj=data},type:"GET"});this.save_to_cache(name,obj.id,obj.versions);return obj};ServerBackend.prototype.get_from_cache=function(name,id){if(this.server_cache[name])if(this.server_cache[name][id])return this.server_cache[name][id]};ServerBackend.prototype.save=function(name,id,versions,force_write){var cached_obj,proposed_obj,status;proposed_obj={id:id,versions:versions};
...
Controller.prototype.bind_actions=function(){var action,fn,hotkey,key,key_parts,selector,_ref,_results;_ref=this.actions;_results=[];for(key in _ref){if(!__hasProp.call(_ref,key))continue;fn=_ref[key];key_parts=key.split(" ");action=key_parts.shift();hotkey="";if(action==="keydown"||action==="keyup"||action==="keypress")hotkey=key_parts.shift();if((hotkey.indexOf("#")!==-1||hotkey.indexOf(".")!==-1)&&hotkey.indexOf("+")===-1){key_parts.shift(hotkey);hotkey=""}selector=key_parts.join(" ");if(selector.length<=
0)selector=document;if(hotkey)_results.push($(selector).bind(action,hotkey,this[fn]));else _results.push($(selector).bind(action,this[fn]))}return _results};Controller.prototype.unbind_actions=function(){return $(document).unbind()};Controller.prototype.html=function(htm){this.container_html=htm;this.container.html(this.container_html);return Flakey.events.trigger("html_updated")};Controller.prototype.make_active=function(){var sub,_i,_len,_ref,_results;this.active=true;this.render();this.bind_actions();
this.container.removeClass("passive").addClass("active");_ref=this.subcontrollers;_results=[];for(_i=0,_len=_ref.length;_i<_len;_i++){sub=_ref[_i];_results.push(sub.make_active())}return _results};Controller.prototype.make_inactive=function(){var sub,_i,_len,_ref,_results;this.active=false;this.unbind_actions();this.container.removeClass("active").addClass("passive");_ref=this.subcontrollers;_results=[];for(_i=0,_len=_ref.length;_i<_len;_i++){sub=_ref[_i];_results.push(sub.make_inactive())}return _results};
Controller.prototype.set_queryparams=function(params){var sub,_i,_len,_ref,_results;this.query_params=params;_ref=this.subcontrollers;_results=[];for(_i=0,_len=_ref.length;_i<_len;_i++){sub=_ref[_i];_results.push(sub.set_queryparams(params))}return _results};return Controller}();Stack=function(){function Stack(config){var contr,name,_i,_len,_ref,_ref2;if(config==null)config={};this.resolve=__bind(this.resolve,this);this.id=this.id||"";this.class_name=this.class_name||"";this.active=this.active||false;
this.controllers=this.controllers||{};this.routes=this.routes||{};this["default"]=this["default"]||"";this.active_controller=this.active_controller||"";this.parent=this.parent||null;this.query_params=this.query_params||{};this.container=$(document.createElement("div"));this.container.attr("id",this.id);_ref=this.class_name.split(" ");for(_i=0,_len=_ref.length;_i<_len;_i++){name=_ref[_i];this.container.addClass(name)}this.container.html(this.container_html);this.parent=config.parent||Flakey.settings.container;
this.parent.append(this.container);_ref2=this.controllers;for(name in _ref2){if(!__hasProp.call(_ref2,name))continue;contr=_ref2[name];this.controllers[name]=new contr({parent:this.container})}window.addEventListener("hashchange",this.resolve,false)}Stack.prototype.resolve=function(){var controller,controller_name,hash,location,name,new_controller,querystring,regex,route,_ref,_ref2;hash=Flakey.util.get_hash();new_controller=void 0;if(hash.length>0){if(hash.indexOf("?")!==-1){hash=hash.split("?");
location=hash[0];querystring=hash[1]}else{location=hash;querystring=""}new_controller=void 0;_ref=this.routes;for(route in _ref){if(!__hasProp.call(_ref,route))continue;controller_name=_ref[route];regex=new RegExp(route);if(location.match(route))new_controller=controller_name}}if(!new_controller){window.location.hash="#"+this["default"];return}this.active_controller=new_controller;this.controllers[this.active_controller].set_queryparams(Flakey.util.querystring.parse(querystring));_ref2=this.controllers;
for(name in _ref2){if(!__hasProp.call(_ref2,name))continue;controller=_ref2[name];if(name!==this.active_controller)this.controllers[name].make_inactive()}if(this.active){this.controllers[this.active_controller].make_active();this.controllers[this.active_controller].render()}return this.controllers[this.active_controller]};Stack.prototype.make_active=function(){this.resolve();if(this.controllers[this.active_controller]!==void 0){this.controllers[this.active_controller].make_active();this.controllers[this.active_controller].render()}return this.active=
true};Stack.prototype.make_inactive=function(){if(this.controllers[this.active_controller]!==void 0)this.controllers[this.active_controller].make_inactive();return this.active=false};Stack.prototype.set_queryparams=function(params){return this.query_params=params};return Stack}();Flakey.controllers={Stack:Stack,Controller:Controller};Template=function(){function Template(eco,name){this.eco=eco;this.name=name}Template.prototype.render=function(context){if(context==null)context={};return this.eco(context)};
return Template}();get_template=function(name,tobj){var template;template=tobj.ecoTemplates[name];return new Template(template,name)};Flakey.templates={get_template:get_template,Template:Template};if(typeof module!=="undefined"&&module!==null)module.exports=Flakey;if(window)window.Flakey=Flakey}).call(this);
